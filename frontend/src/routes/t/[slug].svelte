...
<script>
    ...
    ].concat(Object.entries(task.custom_sections).map(([name, text]) => ({
        name,
        text
    })));
    ...
    $: if (files.length === 0) {
        ...
        files[0].text().then(x => code = x).then(() => {
            rehightlight()
...
{#if code}
    <pre class="border code-preview rounded overflow-auto mt-10">
        <code class={`language-${getGeneralLanguageName(language)}`} id="code-preview"></code>
    </pre>
{/if}
    ...
    <div class="lg:w-1/2 sm:w-2/5 w-4/5 mx-auto flex lg:flex-row flex-col lg:flex-wrap">
        ...
        <div class="w-1/2">
            {#each languageOptions as languageOption}
                <label>
                    <input type="radio" name="lang" value={languageOption.value} bind:group={language} required on:change={rehightlight} />
                    {languageOption.name}
                </label>
            {/each}
        </div>
