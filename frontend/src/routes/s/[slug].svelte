...
<a rel=prefetch class="block w-full bg-red-200 rounded px-5 py-2 mb-5 font-bold hover:bg-red-500" href={`/t/${submission.alias}`}>
...
{:else}
<h3 class="text-lg">Total: <strong><Result points={submission.result.points} status={submission.result.status} /></strong></h3>
{/if}
...
<div class="flex flex-wrap lg:flex-no-wrap mt-5 justify-around">
    ...
    <div class="lg:w-1/2 w-full mt-5 lg:mt-0">
        <pre class="border code-preview rounded overflow-auto">
            {#if submission.code}
                <code class={`language-${getGeneralLanguageName(submission.lang)}`} id="code-preview" style="height: 500px"></code>
            {/if}
        </pre>
    </div>
</div>
