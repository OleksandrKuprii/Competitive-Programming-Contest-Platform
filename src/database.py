...
async def add_results_to_db(results):
    ...
    await conn.executemany('INSERT INTO result (status, points, submission_id,'
                           'test_id, wall_time, cpu_time) '
...
async def get_test_ids(task_id):
    ...
    test_ids = await conn.fetch('SELECT test_id FROM test WHERE task_id = $1',
                                task_id)
...
async def add_submission(submission_to_db):
    ...
    await conn.execute('INSERT INTO submission (date, user_id, task_id, lang,'
